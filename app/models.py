...
class PlagiarismType(enum.StrEnum):
    verbatim = "verbatim"
    near_copy = "near_copy"
    patchwork = "patchwork"
...
class Submission(Base):
    ...
    pairs_as_a = relationship(
        "SimilarityPair",
        foreign_keys="SimilarityPair.submission_a_id",
        back_populates="submission_a",
    )
    pairs_as_b = relationship(
        "SimilarityPair",
        foreign_keys="SimilarityPair.submission_b_id",
...
class SimilarityPair(Base):
    ...
    submission_a = relationship(
        "Submission", foreign_keys=[submission_a_id], back_populates="pairs_as_a"
    )
    submission_b = relationship(
        "Submission", foreign_keys=[submission_b_id], back_populates="pairs_as_b"
    )
    fragments = relationship("MatchedFragment", back_populates="pair", cascade="all, delete-orphan")
    type_result = relationship(
        "PlagiarismTypeResult", back_populates="pair", uselist=False, cascade="all, delete-orphan"
    )
    review = relationship(
        "ReviewDecision", back_populates="pair", uselist=False, cascade="all, delete-orphan"
    )
